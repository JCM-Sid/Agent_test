...
# 1. Création du serveur unique qui contiendra TOUS les outils
mcp_master_server = Server("mon-orchestrateur-mcp")

# 2. On attache les outils des différents modules au serveur maître
# (On réutilise tes fonctions existantes list_xxx et call_xxx)
@mcp_master_server.list_tools()
async def list_combined_tools():
    # On fusionne toutes les listes d'outils
    all_tools = (
        await list_weather_tools() + 
        await list_forecast_tools() + 
        await list_searx_tools() + 
        await list_linkedin_tools() + 
        await list_theirstack_tools() + 
        await list_doctolib_tools()
    )
    return all_tools

@mcp_master_server.call_tool()
async def call_combined_tool(name: str, arguments: dict):
    # Mapping pour rediriger vers la bonne fonction interne
    # On utilise ton dictionnaire tools_map existant dans la classe MCPOrchestrator
    tools_map = {
        "get_current_weather": call_weather_tool,
        "get_forecast": call_forecast_tool,
        "web_search": call_searx_tool,
        "get_my_profile": call_linkedin_tool,
        "post_to_linkedin": call_linkedin_tool,
        "post_from_linkedin": call_linkedin_tool,
        "search_jobs": call_theirstack_tool,
        "doctolib_search": call_doctolib_tool,
    }

    if name in tools_map:
        return await tools_map[name](name, arguments)
    raise ValueError(f"Outil inconnu : {name}")

# 3. Configuration de FastAPI et du transport SSE
app = FastAPI(title="MCP Master Server")
transport = FastApiServerTransport(mcp_master_server, endpoint="/mcp/sse")

@app.get("/")
async def root():
    return {"status": "MCP Server is running", "endpoint": "/mcp/sse"}

@app.on_event("startup")
async def startup():
    await transport.connect()

# Intégration des routes MCP (indispensable pour n8n)
app.include_router(transport.router)

if __name__ == "__main__":
    # Lancement sur le port 6000 de ta machine Windows
